...
resource oci_core_instance export_theia-web {
  agent_config {
  ...
  create_vnic_details {
    ...
    display_name = "theia-web"
    freeform_tags = {
    }
    hostname_label = "theia-web"
    nsg_ids = [
    ]
    private_ip             = "10.0.0.3"
  ...
  source_details {
    ...
    source_id   = var.theia-web_source_image_id
    source_type = "image"
  }
  state = "RUNNING"
}
resource null_resource export_theia-web_file {
  depends_on = [oci_core_instance.export_theia-web]
  
  connection {
    agent       = false
    timeout     = "30m"
    host        = oci_core_instance.export_theia-web.public_ip
    user        = "opc"
...
resource null_resource export_theia-web_remote-exec {
  depends_on = [null_resource.export_theia-web_file]
...
resource null_resource export_theia-web_remote-exec_oracle {
  depends_on = [null_resource.export_theia-web_remote-exec]
